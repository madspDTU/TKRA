...
public class RSUET extends RouteChoiceModel {
	...
	private ConvergencePattern columnGenerationAlgorithm(Network network, RefCostFun phi, RefCostFun omega, String... varargs) {
		...
		while (n <= nmax) { // Outer loop
			...
			gap = gapUsed + gapUnused;
			conv.addIteration(n, gapUnusedBelow, gapUnusedAbove, gapUsed);

			if (!suppress)
				conv.printCurrentIteration();
			if (gap < epsilon && n > 2) {
				n--;
				System.out.println("RSUET terminated with relative gap less than epsilon. Iteration: " + n);
				break;
			}
	...
	private ConvergencePattern cutUniversalChoiceSetAlgorithm(Network network) {
		...
		network.generatePseudoUniversalChoiceSet();
		...
		while (!isConverged && !hasFailed) {
			...
			if (doRedistribution) {
				...
				redistributeFlowOnMarkedRoutesAccordingToProbability(network,1000);
			}
			...
			conv.addIteration(iterationNumber, gapUnusedBelow, gapUnusedAbove, gapUsed);
			conv.printCurrentIteration();
			...
			if (gapUnused <= 0.0001 && gapUsed <= 0.00001) isConverged = true;
